...
def sanityCheck(data):
    ...
    for n in range(len(data)):
        ...
        if length < minlen:
            minlen = length
    print("done. \nThe number of nonempty entries per row ranges from %d to %d\n" %
        (minlen, maxlen))
...
def convertDataTypes(data, typelist, convertFirstRow):
    nSuccessfulTypes = 0
    nFailedTypes = 0
    if typelist==[]:
        return data
    ...
    for n in range(len(data)):
        row = data[n]
        newrow = []
        if n == 0 and not convertFirstRow:
            newrow = row
            nSuccessfulTypes += len(row)
            newdata+=[newrow]
            continue
        ...
        for m in range(len(row)):
            ...
            if typ in ["str", "string", "s"]:
                ...
                truebools_lower = ["1", "true",  "t", "yes"]
                falsebools_lower= ["0", "false", "f", "no"]
            ...
            "numpy.array", "np"]:
                ...
                print("INVALID TYPE. YOU SHOULD NEVER SEE THIS")
                newrow+=[row[m]]
                nFailedTypes+=1
        newdata+=[newrow]
    print("...done.\n")
...
def importcsv(path, colsToKeep=[], typelist=[], convertFirstRow=False,
              doBackupCheck=True):
    if doBackupCheck:
        ans = input("""Ready to import from csv. 
            Have you backed up your csv file(s)? y/n: """)
        if not (ans in ["yes", "yy", "y", 'ye']):
    ...
    newdata = convertDataTypes(newdata,typelist, convertFirstRow)
